...
- 当前上位机里，`payload byte0` 的**波长编号**同时承担了原先“emitter-state”的语义。
- 也就是说：**第 0 位是什么波长，就表示当前样本属于哪个发射状态**。
- 因此后续分段、配对、MBLL 组织都只依赖 `Wavelength`，不再依赖单独的 `Emitter` 字段。
